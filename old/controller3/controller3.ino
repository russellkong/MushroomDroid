...
void setup() {
  ...
  //    rtc.setTime(13, 43, 30);     // Set the time to 12:00:00 (24hr format)
...
void loop() {
  ...
  if (tentMode > 0) {
    ...
    if (tentStep > 0 //confirmed execution
        || confirmProgram()
       ) {
      executeProgram();
    } else {
...
void runTempHi() {
  ...
  if (tentStep == 0) {
    ...
    if (airCon) {
      ...
      if (workingTemp > wetTemp(envTemp, envRh, humidMax)) {
        switchVFan(LOW);
      } else if (workingTemp < wetTemp(envTemp, envRh, humidMax) - 1) {
        switchVFan(HIGH);
      }
      if (workingRh >= humidMin) {
        switchMister(HIGH);
      } else if (workingRh < 70) {
        switchMister(LOW);
      }
    } else {
      //if (workingRh < humidMin || envRh < humidMin) switchMister(LOW);
      switchVFan(LOW);
      //dry intake air allow evaporation, but too dry will dry out mushroom
      if (workingRh < humidMin || envRh < 50) {
        switchMister(LOW);
...
void runRegVent() {
  ...
  if (tentStep == 0) {
    ...
    if (envRh < 0 || envRh <= humidMin) {
      switchMister(LOW);
    } else if (envRh > humidMid) {
      switchMister(HIGH);
    }
...
void runIdle() {
  ...
  if ((envTemp > tempMin + 1 && envTemp < tempMax - 1 &&
    ...
    if (workingCO2 > CO2_MAX) {
      ...
      } else  if (workingTemp < tempMix) {
        if (workingRh > humidMax) {
    ...
    while (confFile.available()) {
      ...
      if (inMode == mode) {
        ...
        sprintf_P(tmpLog, STR_CONF,  mode, humidMin, humidMid, humidMax, tempMin, tempMid, tempMax,
                  ventInv, ventDur, lightStart, lightEnd, wetHour, wetDur, airCon ? 'Y' : 'N'); addLog(tmpLog);
        break;
      }
    }
  } else {
